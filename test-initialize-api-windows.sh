...
# Test script for /users/initialize API endpoint (Windows-compatible)
# This script tests the corrected curl command with proper JWT token format
# Uses --insecure flag to bypass SSL certificate verification issues on Windows
...
# Make the API call with --insecure flag to bypass SSL certificate issues
curl -i --insecure \
  -X POST "$API_URL" \
